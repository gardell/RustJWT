...
mod jwt {
    ...
    fn validate_header(header: Header) -> Result<()> {
        try!(if header.alg == "HS256" {
            Ok(())
        } else {
            Err(Error::Format)
        });
        try!(if header.typ == "JWT" {
            Ok(())
        } else {
            Err(Error::Format)
        });
    ...
    pub fn parse<T: ::serde::de::Deserialize>(json: &str, key: &[u8]) -> Result<T> {

        let mut rparts = json.rsplitn(2, |c| c == '.');
        try!(match (rparts.next().ok_or(Error::Format).and_then(base64_decode), rparts.next()) {
            (Ok(signature), Some(message)) => {
                let hmac_equals = hmac_sha256_equals(message.as_bytes(), key, signature.as_slice());
                if hmac_equals {
                    Ok(())
                } else {
                    Err(Error::Signature)
        ...
        match (parts.next().ok_or(Error::Format).and_then(base64_decode),
               parts.next().ok_or(Error::Format).and_then(base64_decode)) {
            (Ok(header), Ok(payload)) => {
                try!(parse_json(header.as_slice()).and_then(validate_header));
...
mod tests {
    ...
    fn serialize_header() {
        ...
        assert_eq!(deserialized.get("alg"),
                   Some(&::serde_json::Value::String(ALG.to_string())));
        assert_eq!(deserialized.get("typ"),
    ...
    fn deserialize_header() {
        ...
        let deserialized: ::jwt::Header = match ::serde_json::from_slice(serialized) {
            Ok(header) => header,
            _ => panic!("unable to deserialize"),
        };
    ...
    fn deserialize_header_unknown_field() {
        let serialized = br#"{ "unknown": "value", "typ": "Jedi", "alg": "Yoda" }"#;
    ...
    fn parse_valid() {
        let payload: ::Payload =
            ::jwt::parse("eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.\
                          eyJzdHJpbmciOiJCaWxibyBCYWdnaW5zIiwiaW50ZWdlciI6MTMzN30.\
                          hKRaWXYKNMRdxicE23jPHyH6W7mt4G491YXgf4LWHKs",
                         "secret".as_bytes())
                .unwrap();
